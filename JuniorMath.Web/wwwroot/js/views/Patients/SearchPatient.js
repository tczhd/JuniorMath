$(document).ready(function () {
    ...
    $('#search').click(function () {
        JuniorMath.Patient.SearchPatient($(this), 1);
    });


    $('#add').click(function () {
        JuniorMath.Patient.AddSearchFilter();
    });
                ...
                if (result.success) {
                    ...
                    if (result.patient_detail.length > 0) {
                        $.each(result.patient_detail, function (index, patient) {
                            var tr = JuniorMath.Patient.GetPatientRow(patient);
                            bodyPatients.append(tr);
                        });

                        JuniorMath.setModalPagination("JuniorMath.Patient.SearchPatient", result.current_page, result.pages, result.total_pages);
                    }
                    else {
