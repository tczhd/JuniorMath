var JuniorMath = {
    ...
    getModalFooterButton: function (id, buttonName) {
        var button = JuniorMath.getModalFooterButtonString(id, buttonName);
        return $(button);
    },
    ...
    setModalPagination: function (methodName, currentPage, pages, totalPages) {
        ...
        if (currentPage > 1) {
            pagination += JuniorMath.getPaginationItem(1, "First", false);

            pagination += JuniorMath.getPaginationItem(currentPage - 1, "Previous", false);
        }
        ...
        $.each(pages, function (index, val) {
            ...
            pagination += JuniorMath.getPaginationItem(val, "" + val, active);
        });

        if (currentPage < totalPages) {

            pagination += JuniorMath.getPaginationItem(currentPage + 1, "Next", false);

            pagination += JuniorMath.getPaginationItem(totalPages, "Last", false);
        }
        ...
        $(liPages).each(function () {
            ...
            $(link).click(function (event) {
                ...
                JuniorMath.executeFunctionByName(methodName, $(link), parseInt(pageIndex));
            });
        });
    },
